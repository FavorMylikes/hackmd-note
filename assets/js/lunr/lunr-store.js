var store = [{
        ...
        "excerpt":"准备看点摄像机视觉 Week1 ![20210727143124](https://raw.githubusercontent.com/FavorMylikes/hackmd-note/img/img/20210727143124.png =400x) Camera Modeling ![20210727113340](https://raw.githubusercontent.com/FavorMylikes/hackmd-note/img/img/20210727113340.png =400x) $\\frac{1}{f} = \\frac{1}{a} + \\frac{1}{b}$ $f$焦距， $a$, 物体到透镜的位置, $b$, 透镜到成像的位置 $\\frac{Y}{a}=\\frac{y}{b}$ $Y$物体大小, $y$像的大小 Single View Geometry 平面图到透视图 ![20210727142444](https://raw.githubusercontent.com/FavorMylikes/hackmd-note/img/img/20210727142444.png =400x) $o^*$为眼睛位置 $S$为垂足 $OS=O^S$, 因此$\\angle{SOO^}=45\\degree$ $P$为平面图上一点 连接$P-O$交图像平面$M$ 连接$P-O^$交图像平面$P^$ $\\triangle MLP^$相似于$\\triangle OSO^$ $\\angle LMP’$为$45\\degree$ $\\triangle MLP^=\\triangle MLP’$, 即，将成像面的点$P^$映射到平面$P’$变为透视图 因此，$P’$是$P$关于$ML$对称的透视图 多透视点画法Kim Jung Gi ![20210727155712](https://raw.githubusercontent.com/FavorMylikes/hackmd-note/img/img/20210727155712.png =400x) 投影面[Projective...","categories": ["coursera","3D"],
        "tags": [],
        "url": "/hackmd-note/coursera/3d/2021/07/27/Robotics-Perception-week-1.html",
        "teaser": null
      },{
        "title": "Jekyll+Github page+Gihub Actions",
        "excerpt":"尝试使用github pages 主要组件 Jekyll 静态网站生成器 Github Actions 需要项目下存在.github/workflows/*.yml, 作为Github Actions的扫描程序 Action marketplace _config.yml, 根目录下，Jekyll运行时需要的配置 ruby Gemfile ruby 需要的版本 Gemfile.lock ruby 一台机器已经安装各个组件的具体版本以及依赖条件 运行流程 ![1627526442(1)](https://i.loli.net/2021/07/29/weIDpHRvW9XB5Cu.png =400x) Actions扫描workflows下的配置文件寻找job Actions按照顺序依次执行 ruby 按照Gemfile.lock的文件安装指定组件版本 ruby拿到_config.yml的配置之后依次执行 主题 dash 插件 jekyll-feed jekyll/tagging jekyll-paginate paginate: 5 # 分页数 paginate_path: “/hackmd-note/page:num/” # 扫描位置 kramdown-math-katex 参考 minusgix.github.io ![20210729105618](https://i.loli.net/2021/07/29/dvO7q5fJcZ8CKVg.png =400x) git 南方科技大学数学建模协会...","categories": ["tutorial"],
        "tags": [],
